...
Deploy the **GLM-5.2-FP8** model onto Modal **8× B200** GPUs, pulling the image from a private
...
# 4. Populate the weights Volume once (GLM-5.2-FP8 + DFLASH draft). Both
...
The Modal app name is `APP_NAME` (currently `glm-5-2-fp8-marathon`). Most lifecycle
...
uv run modal app logs glm-5-2-fp8-marathon    # stream logs (the app name from deploy.py)
uv run modal app history glm-5-2-fp8-marathon # deployment versions
uv run modal app dashboard glm-5-2-fp8-marathon  # open the dashboard page

# Stop / roll
uv run modal app stop glm-5-2-fp8-marathon     # permanently stop + terminate containers
uv run modal app rollover glm-5-2-fp8-marathon # redeploy to get fresh containers, no code change
uv run modal app rollback glm-5-2-fp8-marathon # go back to the previous deployment
...
- **Host RAM cap:** Modal limits a container to **1,650,688 MiB (~1.575 TiB)**
  ...
  **1,363,149 MiB (1.3 TiB)** to make 8× B200 scheduling easier.
- **CPU cap:** 64 physical cores max (`CPU = 64.0`).
- **Cold start:** GLM-5.2 fp8 load + cuda-graph(bs=64) build is slow;
