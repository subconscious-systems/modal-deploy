"""
Deploy GLM-5.3-FP8 (model name "glm-5-3-fp8-marathon") onto Modal 8x B200s.
...
2. Populate the weights Volume once from Hugging Face:
   ...
   Serve reads local paths `/models/glm-5.3-fp8` — no Hub fetch at GPU startup.
3. `uv run modal deploy deploy_glm5_3.py` -> Modal pulls the serve image (auth via
   DOCKER_TOKEN_SECRET), mounts the weights + warm-cache volumes, and runs
   the sglang launch server on Nx B200.
   `uv run modal serve deploy_glm5_3.py` does the same but hot-reloads for dev.
"""
...
#   pre-downloaded onto the Volume by scripts/download_weights_glm5_3.py; the serve
#   container reads them from local paths under /models.

# Volumes
WEIGHTS_VOLUME = "glm_weights_vol"      # GLM-5.3 checkpoint under /models
...
STARTUP_TIMEOUT = 3000            # GLM-5.3 fp8 load + cuda-graph(bs=64) build is slow
...
image = (
    ...
    .env(
        {
            # So the container re-import of deploy_glm5_3.py sees the same value
            ...
            # Hugging Face cache persists on the volume; /tmp is ephemeral on Modal.
...
def serve():
    ...
    cmd = [
        ...
        "--subconscious-x-st-buffer-size", "15",
